...
class Mage:
    ...
    def hero_status(self, HP, AR, DF, MN):
        print (f"Masz {HP} HP, {AR} AR, {DF} DF oraz {MN} MN.")
    ...
    def prepare(self, AR):
        print(f"Jesteś przygotowany do ataku w następnej turze. Punkty obrażeń wyrosły do {AR} AR.")
    ...
    def use_spell_fireball(self, target, target_HP, MN):
        print(f"Rzucasz zaklęcie Fireball na {target}a, {target} ma {target_HP} HP. Masz {MN - 4} MN.")

    def use_spell_blessing(self, HP, MN):
        print(f"Wywołujesz zaklęcie Blessing. Masz {HP + 5} HP oraz {MN - 5} MN.")
